...
class Neo4jIndexProvider(IIndexProvider):
    ...
    def get_functions_in_cluster(
        self, cluster_id: int, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            with self.driver.session() as session:
                ...
                logger.info(
                    f"Retrieved {len(functions)} functions from cluster {cluster_id}"
                )
                return functions
    ...
    def get_functions_with_most_cross_file_calls(
        self, limit: int = 20
        ...
        try:
            with self.driver.session() as session:
                ...
                logger.info(
                    f"Retrieved {len(outgoing)} functions with most outgoing cross-file calls"
                )
                ...
                logger.info(
                    f"Retrieved {len(incoming)} functions with most incoming cross-file calls"
                )
    ...
    def search_files(self, pattern: str) -> List[str]:
        ...
        try:
            ...
            logger.debug(
                f"Converted glob pattern '{pattern}' to regex '{regex_pattern}'"
            )
...
class Neo4jIndexManager(IIndexManager):
    ...
    def find_files(self, pattern: str = "*") -> List[str]:
        ...
        with self._lock:
            if not self.index_provider:
                logger.warning(
                    f"Cannot find files matching '{pattern}': Index provider not initialized"
                )
                return []
    ...
    def get_file_summary(self, file_path: str) -> Optional[Dict[str, Any]]:
        ...
        with self._lock:
            if not self.index_provider:
                logger.warning(
                    f"Cannot find files matching '{file_path}': Index provider not initialized"
                )
                return None
    ...
    def set_clustering_config(self, enabled=True, k=5, max_iterations=50):
        ...
        with self._lock:
            try:
                ...
                logger.info(
                    f"Set clustering configuration: enabled={enabled}, k={k}, max_iterations={max_iterations}"
                )
                return True
    ...
    def run_kmeans_clustering(self, k, max_iterations, embedding_dimensions, ctx=None):
        with self._lock:
            if not self.index_builder:
                logger.error("Index builder not initialized")
                return {"error": "Index builder not initialized"}

            try:
                logger.info("Running Neo4j gds K Means...")
                return self.index_builder.run_kmeans_clustering(
                    k, max_iterations, embedding_dimensions
                )
            except Exception as e:
                logger.error(f"Failed to run k means: {e}")
                return {"error": e}
        ...
        if self.config_path and os.path.exists(self.config_path):
            try:
                ...
                if "clustering" in config:
                    ...
                    self.clustering_max_iterations = clustering_config.get(
                        "max_iterations", 50
                    )
                ...
                logger.warning(
                    f"Failed to load Neo4j configuration from {self.config_path}: {e}"
                )
